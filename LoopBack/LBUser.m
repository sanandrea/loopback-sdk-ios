...
- (LBUser *)createUserWithUserName:(NSString*)username
                          password:(NSString*)password{
    LBUser *user = (LBUser *)[self modelWithDictionary:nil];
...
- (void)loginWithUserName:(NSString*)username
    ...
    [self invokeStaticMethod:@"login"
                  parameters:nil
              bodyParameters:@{@"username": username, @"password": password}
                     success:^(id value) {
                         NSAssert([[value class] isSubclassOfClass:[NSDictionary class]], @"Received non-Dictionary: %@", value);
                         LBRESTAdapter* adapter = (LBRESTAdapter*)self.adapter;
                         if (self.accessTokenRepository == nil) {
...
- (void)userByLoginWithUserName:(NSString*)username
    ...
    NSParameterAssert(username);
    NSParameterAssert(password);
    [self loginWithUserName:username password:password success:^(LBAccessToken* token){
        [self findById:token.userId success:^(LBModel *model){
            self.cachedCurrentUser = (LBUser*)model;
            success((LBUser*)model);
        } failure:failure];
